    ...
    <div class="bg-gray-100 p-5 rounded-xl">
        <form action="{{ route('admin.band.social.update', ['band' => $band->id, 'social' => $social->id]) }}" method="POST" enctype="multipart/form-data" class="w-full mx-auto">
            ...
            <div class="mb-5">
                ...
                <input type="text" name="platform" id="platform" value="{{ $social->platform }}" class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Nama Platform (misal: Instagram)" required />
            </div>
            
            <div class="mb-5">
                <label for="url" class="block mb-2 text-sm font-medium text-gray-900">URL Social Media</label>
                <input type="text" name="url" id="url" value="{{ $social->url }}" class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="URL Social Media" required />
            </div>
