        ...
        <form action="{{ route('admin.band.update', ['band' => $band->id]) }}" method="POST" enctype="multipart/form-data" class="w-full mx-auto">
            ...
            <div class="lg:grid lg:grid-cols-2 gap-5">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="name" name="name" value="{{ $band->name }}" class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Nama Band" required />
                </div>
            </div>
            <div class="mb-5">
                <label for="description" class="block mb-2 text-sm font-medium text-gray-900">Deskripsi</label>
                <textarea id="description" name="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-xl border border-gray-300 focus:ring-green-500 focus:border-green-500">{{ $band->description }}</textarea>            
            </div>
