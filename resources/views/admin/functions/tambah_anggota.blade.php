    ...
    <div class="bg-gray-100 p-5 rounded-xl">
        <form action="{{ route('admin.band.anggota.store', ['band' => $band->id]) }}" method="POST" enctype="multipart/form-data" class="w-full mx-auto">
            ...
            <div class="mb-5">
                ...
                <select id="jumlah_anggota" name="jumlah_anggota"
            ...
            <div class="mb-5 lg:grid lg:grid-cols-2 gap-5">
                @for ($i = 1; $i <= 4; $i++)
                    <div id="anggota{{ $i }}" class="anggota" style="display: none;">
                        ...
                        <div class="mb-3">
                            <label class="block mb-2 text-sm font-medium text-gray-900" for="foto_anggota{{ $i }}">Upload Foto
                                Anggota</label>
                            <input
                                class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50"
                                id="foto_anggota{{ $i }}" type="file" name="members[{{ $i - 1 }}][image]">

                        </div>
                        <div class="lg:grid lg:grid-cols-2 gap-5">
                            <div class="mb-5">
                                <label for="nama_anggota{{ $i }}" class="block mb-2 text-sm font-medium text-gray-900">Nama
                                    Anggota</label>
                                <input type="text" id="nama_anggota{{ $i }}" name="members[{{ $i - 1 }}][name]"
                                    class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    placeholder="Nama Anggota" required />

                            </div>
                            <div class="mb-5">
                                <label for="posisi_anggota{{ $i }}" class="block mb-2 text-sm font-medium text-gray-900">Posisi
                                    Band</label>
                                <input type="text" id="posisi_anggota{{ $i }}" name="members[{{ $i - 1 }}][role]"
                                    class="shadow-xs bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    placeholder="Posisi di Band" required />
            ...
            <div class="flex flex-row space-x-2">
                <a href="{{ route('admin.band.index') }}">
                    <button type="button"
                        class="text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-xl text-sm px-5 py-2.5 text-center cursor-pointer">Kembali</button>
                </a>
                <button type="submit"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-xl text-sm px-5 py-2.5 text-center cursor-pointer">Buat
                    Data</button>
            </div>
        </form>
    </div>
